...
def run_mission(prompt_file=None, model_id=DEFAULT_MODEL, persona_path=DEFAULT_PERSONA):
    ...
    if os.path.exists(persona_path):
        ...
        # CRITICAL FIX: Override the "Teacher/Interactive" instructions
        system_instruction = (
            f"{raw_persona}\n\n"
    ...
    if prompt_file:
        ...
        user_prompt = "Calculate the md5 sum of the file 'auto_test_agent.py' and print it."
    ...
    chat = client.chats.create(
        ...
        config=types.GenerateContentConfig(
            ...
            automatic_function_calling=types.AutomaticFunctionCallingConfig(
                disable=False,
                maximum_remote_calls=20 # Increased limit for complex tasks
            )
        )
    )

    # 5. Execute
    # We append a reminder to the prompt to ensure the model doesn't drift
